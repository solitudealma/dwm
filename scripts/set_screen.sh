...
:<<!
  ...
  one: 只展示一个内置屏幕 1920x1080 缩放为 1440x900
...
__get_inner_view() {
    case "$MODE" in
        ONE) echo "--output $INNER_PORT --mode 1920x1080 --pos 0x0" ;;
        LR) echo "--output $INNER_PORT --mode 2880x1800 --pos 1920x320" ;;
        TB) echo "--output $INNER_PORT --mode 2880x1800 --pos 500x1080" ;;
        HLR) echo "--output $INNER_PORT --mode 2880x1800 --pos 2560x320" ;;
        HTB) echo "--output $INNER_PORT --mode 2880x1800 --pos 500x1080" ;;
    esac
}
